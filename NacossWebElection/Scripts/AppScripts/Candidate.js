        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.value === 200) {
                    ...
                    var row = '<tr id="' + rowID + '"> <td>' + cn + '</td> <td>' + response.post.FirstName + " " + response.post.LastName + '</td><td>' + response.position + '</td><td>' + response.post.CurrentLevel + '</td><td>' + response.post.MatNo + '</td><td><a href="" class="btn btn-xs btn-primary">Edit</a> | <a href="' + serverpost + '" data-formated-id="' + formatedId + '" class="btn btn-xs btn-default delete">Delete <img src="/Images/ajax/LoadingCircle.gif" class="hide-loader" id="' + imgID + '" width="20" height="20"></a></td> </tr>';
                    $("tbody").append(row);
                    $("#msgText").text(response.message);
                    url = $(location).attr("origin");
                    url2 = response.url;
                    redirect = url + url2;
                    $("#UploadNext").attr("href", url2)
                    $("#myModal").modal("show");
